...
const posts = [
  {
    ...
    comment: "Not so sure about the acid I just took.",
    likes: 21492
  },
  ...
  
  {
    ...
    comment:
      "That feeling when you pass a mirror and you know you're looking good today",
    likes: 15137
  }
]

let loadingEffect = setInterval(generatePosts, 800)

//this whole thing works a lot like a manually created "for" loop. The point is just to create a "loading" effect where each post comes in gradually (every 1000 ms, as defined by the setInterval above) and the user can see that it is being generated dynamically by the JavaScript
...
function generatePosts() {
  main.innerHTML += `<div class="overall-post-container" id="post-container${i}">
            ...
            <div class="bottom-info" id="bottom-info${i}">
                ...
                <p class="comment"><span class="user-name">${
                  posts[i].username}</span> ${posts[i].comment}</p>
            </div> 
  ...
  document.getElementById(`post-container${i}`).style.animation =
    "fadeInEffect 1s linear"

  window.scrollBy({
    top: window.innerHeight,
    behavior: "smooth"
  }) //to scroll down smoothly each time a new post is "loaded"
...
// I'm doing a lot of funky things with the event listeners here, the reason being that I can't just add them the normal way, because the html elements don't exist in the DOM until after they're generated by the JavaScript. I'm basically attaching listiners to the entire document and then setting their conditions so that they only activate when the thing I'm targeting (which doesn't exist until the JavaScript generates it) is clicked. There appear to be some much simpler ways of doing this, but they involved syntax and functions I don't understand yet, and I didn't want to just copy and paste code I wanted to do what I could with what I already know, even if it's much less than ideal.
...
document.addEventListener("click", function (event) {
  ...
  function leaveComment() {
    if (document.getElementById(`comment-input${i}`).value.trim() === "") {
      ...
      document
        .getElementById(`comment-input${i}`)
        .setAttribute("placeholder", "Add a comment as FakeDaniel77")
    } else {
      let comment = document.getElementById(`comment-input${i}`).value
      document.getElementById(
        `bottom-info${i}`
      ).innerHTML += `<p class="comment"><span class="user-name">FakeDaniel77</span> ${comment}</p>`
      document.getElementById(`comment-input${i}`).value = ""
      document
        .getElementById(`comment-input${i}`)
        .setAttribute("placeholder", "Add a comment as FakeDaniel77")
...
function registerLike() {
  document
    .getElementById(`heart-icon${i}`)
    .setAttribute("src", "images/icon-heart-liked.png")
  posts[i].likes++
  document.getElementById(`likes${i}`).textContent = posts[
    i
...
document.addEventListener("click", function (event) {
  if (event.target.className === `icon dm-icon`) {
    console.log(
      "Sorry, Oldagram is currently experiencing techinical difficulties, and messaging is not available."
    )
  }
})
